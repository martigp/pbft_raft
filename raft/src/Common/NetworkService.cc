...
void NetworkService::populateSockAddr(const std::string& addr,
  ...
  if (inet_pton(AF_INET, ip.c_str(), &(sockaddr->sin_addr)) <= 0) {
    throw Exception("Provided ip in " + addr +
...
NetworkService::NetworkService(NetworkUser& user)
    : connectionStateMap(), connectionStateMapLock() {
  userCallbackFunction =
      std::bind(&NetworkUser::handleNetworkMessage, &user,
...
void NetworkService::sendMessage(const std::string sendToAddr,
                                 const std::string msg, bool createConnection) {
    ...
    if (connectedToRecipient) {
      ...
      if (sendSocketFd < 0) {
        std::cerr << "Couldn't create socket for new connection with"
                  << sendToAddr << " with error " << std::strerror(errno)
                  << std::endl;
        return;
      }
      ...
      try {
        ...
        std::cerr << e.what() << ": closing socket to addr " << sendToAddr
                  << std::endl;
        close(sendSocketFd);
        return;
      }
      ...
      if (connect(sendSocketFd, (struct sockaddr*)&(sendToSockAddr),
        ...
        std::cerr << "[Network] Failed to connect with host " << sendToAddr
...
void NetworkService::monitorSocketForEvents(int socketFd) {
  ...
  if (kevent(pollFd, &newEv, 1, NULL, 0, NULL) == -1) {
    std::string errorMsg =
        "Failed to register socket " + std::to_string(socketFd) + "to kqueue ";
    throw Exception(errorMsg + std::strerror(errno));
  }
}

void NetworkService::removeConnection(const std::string& hostAddr) {
  connectionStateMapLock.lock();
  ...
  if (hostInfoPair != connectionStateMap.cend()) {
    ...
    if (connectionState->socketFd != INVALID_SOCKET_FD) {
      ...
      // closed.
      struct kevent ev;
      EV_SET(&ev, connectionState->socketFd, EVFILT_READ, EV_DELETE, NULL, 0,
             NULL);
      if (kevent(pollFd, &ev, 1, NULL, 0, NULL) < 0) {
        throw Exception("Failed to remove " +
                        std::to_string(connectionState->socketFd) +
                        "from kqueue");
      }
    ...
    // This is OK because indicates race condition avoided.
    std::cerr << "[Network] Attempted to remove connection to " << hostAddr
              << " but connection did not exists" << std::endl;
    connectionStateMapLock.unlock();
  }
}

void NetworkService::handleReceiveEvent(int receiveSocketFd,
                                        uint64_t eventInformation,
                                        bool isListenSocket) {
  // Handle event on a listen socket by attempting to create new connection
  // and add relevant connection state to the Network Service
  if (isListenSocket) {
    ...
    int hostSocketFd =
        accept(receiveSocketFd, (struct sockaddr*)&hostSockAddr, &hostAddrLen);
    ...
    if (connectionStateMap.find(hostAddr) != connectionStateMap.end()) {
      close(hostSocketFd);
      std::cerr << "Received duplicate request from host addr " << hostAddr
                << ". Ignoring Conneciton request." << std::endl;
      return;
    }
  ...
  while (totalBytesRead < socketBytesAvailable) {
    ...
    if (connectionState->msg.size() < HEADER_SIZE) {
      // Only read in header bytes worth of bytes from socket
    ...
    if (connectionState->payloadBytesNeeded == UNKNOWN_NUM_BYTES) {
      if (connectionState->msg.size() == HEADER_SIZE) {
        ...
        memcpy(&networkOrderedPayloadLen, connectionState->msg.c_str(),
               HEADER_SIZE);
...
int NetworkService::createListenSocket(const std::string& listenAddr) {
  ...
  int opt = 1;
  if (setsockopt(listenSocketFd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt))) {
    errorMsg =
        "Failed to setsockopt to reuse local addresses"
  ...
  if (bind(listenSocketFd, (struct sockaddr*)&listenSockAddr,
    ...
    errorMsg = "Failed to bind provided listen address " + listenAddr +
               " to the created listen socket: ";
    throw Exception(errorMsg + std::strerror(errno));
  }
  ...
  try {
    ...
    errorMsg =
        "Failure to register the listen socket to the"
        "kqueue: %s",
    err.what();
...
void NetworkService::startListening(const std::string& listenAddr) {
  int listenSocketFd;
  if (listenAddr == "") {
  ...
  while (true) {
    ...
    for (int i = 0; i < numEvents; i++) {
      ...
      if (ev.flags & EV_ERROR) {
        errorMsg = "Kqueue error when polling the %s event id: %s",
