...
ShellStateMachine::ShellStateMachine(
    std::function<void(uint64_t, std::string*)> callbackFn)
...
void ShellStateMachine::stateMachineLoop() {
  ...
  while (true) {
    ...
    printf("[Log State Machine]: Popped RaftClient command: %s",
           smCmd.command.c_str());
    ...
    try {
      ...
      std::cerr << "Failed to apply log entry " << smCmd.index << std::endl;
    }
