...
namespace Common {
    ...
    bool KeyValueStorage::set(std::string key, std::string value) {
        ...
        if (file.fail()) {
            std::string errorMsg = "[KeyValueStorage.cc] Failed to write file " + filepath + " , key-value pair " + key + "-" + value + ": ";
    ...
    bool KeyValueStorage::get(std::string key, std::string& value) {
        ...
        if (file.fail()) {
            std::string errorMsg = "[KeyValueStorage.cc] Failed to read file " + filepath + " , key-value pair " + key + "-" + value + ": ";
            throw std::runtime_error(errorMsg + std::strerror(errno));
        }
        return true;
    }
